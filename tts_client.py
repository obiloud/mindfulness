...
def recursive_word_chunker(text: str, max_words: int) -> list[str]:
    ...
    def split_recursive(text_segment, delimiter_idx):
        ...
        for part in parts:
            ...
            if not part:
                continue
            potential = (current_accumulation + " " +
                         part).strip() if current_accumulation else part
    ...
    if chunks:
        cur = chunks[0]
        for nxt in chunks[1:]:
            if get_word_count(cur) + get_word_count(nxt) <= max_words:
                cur += " " + nxt
            else:
                final.append(cur)
                cur = nxt
...
def estimate_max_tokens(text: str) -> int:
    ...
    chuckles = len(re.findall(
        r'<chuckle>|<excited>|<gasp>', text, re.IGNORECASE))
...
class AudioStreamer:
    def __init__(self, tts_description: str = DESCRIPTION_DEFAULT):
        ...
        retries = Retry(total=8, backoff_factor=1, status_forcelist=[
                        500, 502, 503, 504], allowed_methods=frozenset(['POST', 'GET']), read=True)
        adapter = HTTPAdapter(
            max_retries=retries, pool_connections=CONCURRENT_REQUESTS, pool_maxsize=CONCURRENT_REQUESTS)
            ...
            if isinstance(item, float):
                ...
                for c in chunks:
                    pipeline_items.append(
                        {'type': 'text', 'content': re.sub(pattern=r, repl=" ", string=c)})
    ...
    def _request_audio_chunk(self, text_chunk, chunk_index):
        ...
        payload = {"description": self.tts_description,
                   "text": text_chunk, "max_tokens": max_tokens, "temperature": 0.1}
        try:
            response = self.session.post(
                SERVER_URL, json=payload, timeout=TTS_TIMEOUT)
    ...
    def _crossfade_chunks(self, new_chunk_bytes):
        if not new_chunk_bytes:
            return b""
        new_audio = np.frombuffer(
            new_chunk_bytes, dtype=np.int16).astype(np.float32)

        if len(new_audio) < (2 * CROSSFADE_SAMPLES):
            if self.previous_chunk_tail is not None:
                combined = np.concatenate(
                    (self.previous_chunk_tail, new_audio))
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=CONCURRENT_REQUESTS) as executor:
            ...
            for i in range(len(pipeline_items)):
                ...
                if item['type'] == 'pause':
                    ...
                    if self.previous_chunk_tail is not None:
                        self.audio_queue.put(
                            self.previous_chunk_tail.astype(np.int16).tobytes())
                        self.previous_chunk_tail = np.zeros(
                            CROSSFADE_SAMPLES, dtype=np.int16)    # Reset crossfader

                    # 2. Inject Silence
                    duration = item['duration']
                    logger.info(
                        f"[System] Injecting {duration}s silence (Client-side).")
                    ...
                    for j in range(0, len(silent_bytes), AUDIO_CHUNK_SIZE):
                        self.audio_queue.put(
                            silent_bytes[j:j+AUDIO_CHUNK_SIZE])
                        self.total_buffered_bytes += len(
                            silent_bytes[j:j+AUDIO_CHUNK_SIZE])

                    # Skip to next item (no network request to wait for)
                    continue
                ...
                if i in future_to_index:
                    ...
                    if audio_bytes:
                        ...
                        for j in range(0, len(processed), AUDIO_CHUNK_SIZE):
                            self.audio_queue.put(
                                processed[j:j+AUDIO_CHUNK_SIZE])
                            self.total_buffered_bytes += len(
                                processed[j:j+AUDIO_CHUNK_SIZE])
                ...
                if next_idx < len(pipeline_items):
                    ...
                    if future:
                        future_to_index[next_idx] = future
    ...
    def play_stream(self):
        self.stream = self.p.open(format=FORMAT, channels=CHANNELS,
                                  rate=RATE, output=True, frames_per_buffer=AUDIO_CHUNK_SIZE)
        state = "BUFFERING"
        while True:
            if state == "BUFFERING":
...
if __name__ == "__main__":
    ...
    pipeline = RunnableParallel(
        description=voice_character_chain, text=meditation_guide_generator_chain)
